...
public class Example implements Consumer<Event> {
    ...
    public static Font FONT12 = new Font(FontMgr.getDefault().matchFamilyStyleCharacter("", FontStyle.NORMAL,
            null, "↑".codePointAt(0)), 12);
    public static Font FONT24 = new Font(FontMgr.getDefault().matchFamilyStyle("", FontStyle.NORMAL), 24);
    public static Font FONT48 = new Font(FontMgr.getDefault().matchFamilyStyle("", FontStyle.BOLD), 48);
    ...
    public Options progressBars = new Options("Default", "0%",
            "50%", "100%", "Indeterminate");
    ...
    public Example() {
        ...
        switch (count % 5) {
            case 0 ->
                    window.setWindowPosition(bounds.getLeft() + bounds.getWidth() / 4, bounds.getTop() + bounds.getHeight() / 4);
            ...
            case 4 ->
                    window.setWindowPosition(bounds.getLeft() + bounds.getWidth() / 2, bounds.getTop() + bounds.getHeight() / 2);
        }

        switch (Platform.CURRENT) {
            case WINDOWS -> {
                ///System.out.println(Objects.requireNonNull(getClass().getResource("windows.ico")).toString());
                //System.out.println(Objects.requireNonNull(getClass().getResource("windows.ico")).getPath());
                window.setIcon(new File(Objects.requireNonNull(getClass().getResource("windows.ico")).getPath()));
            }
            case MACOS -> {
    ...
    public void accept(Event e) {
        ...
        if (e instanceof EventKey eventKey) {
            if (eventKey.isPressed() && eventKey.isModifierDown(MODIFIER)) {
                switch (eventKey.getKey()) {
                    ...
                    case N -> new Example();
                    case H -> window.setVisible(false);
                    case W -> window.close();
                    case F -> window.setFullScreen(!window.isFullScreen());
                    case O -> window.setOpacity(window.getOpacity() == 1f ? 0.5f : 1f);
                    case UP -> window.maximize();
                    case DOWN -> window.restore();
                    case M -> window.minimize();
                    case B -> setProgressBar(progressBars.next());
                }
            }
